...
export default function FloorDisplay() {
  ...
  // Auto-refresh every 10 minutes — no user interaction needed
  useEffect(() => {
    ...
    <div className="floor-wrap">
      <div className="floor-header">
        <div>
          ...
          <div className="floor-subtitle">Simulated time: 2026-08-17 09:15</div>
        </div>
        <div className="floor-meta">
          {error
            ? <span className="floor-error">⚠ {error}</span>
            : lastUpdated && (
                <span className="floor-updated">
      ...
      <div className="floor-grid">
        {snapshot.map(d => (
          <MachineCard key={d.machine.machine_code} data={d} />
        ))}
      </div>
    </div>
  )
